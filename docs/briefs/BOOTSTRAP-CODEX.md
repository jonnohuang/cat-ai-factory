# Cat AI Factory — Chat Bootstrap (CODEX / VS Code Extension)

Paste this as the second message in a new CODEX chat inside VS Code
(after the BASE message) when working on an implementation task.

------------------------------------------------------------

Role: **CODEX — Implementation Only (VS Code Extension Agent)**
...
- Documentation editing (human-owned; see CAF-Safe SOP).
...
- You MUST treat the repo as the single source of truth.
- If the PR prompt provides a file allowlist, treat that as authoritative.
...
- Local Planner/gateway runs in Docker.
- Local testing commands run inside the conda env.
- LOCAL determinism remains the gold standard.
...
- Any Cloud mapping must preserve the same plane boundaries and file-bus semantics.
...
- **Hard Restriction:** Do NOT add `gcloud` or `gsutil` calls inside core factory logic (Planner, Control Plane, Worker).
  Use SDKs for logic; CLI is for deployment/ops only.
...
Before implementing anything, read these in order (read-only):
...
- Editing docs is disallowed unless the PR prompt explicitly allowlists the exact doc + exact section.
...
- DO NOT modify `repo/shared/job.schema.json` or ADRs without ARCH approval.
...
- Worker must remain deterministic. No LLM usage or network generation APIs.
...
- NO “summarizing”
- Only add the smallest necessary snippet exactly where instructed.
...
Proposal Contents:
1) **Target Files:** List of all paths to be modified/created.
