...
# Load schema once
SCHEMA_PATH = pathlib.Path(__file__).resolve().parents[3] / "repo" / "shared" / "plan_request.v1.schema.json"
...
async def ingress_plan(
    ...
    if not payload.get("received_at"):
        payload["received_at"] = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
