...
def main() -> int:
    ...
    out_dir = pathlib.Path(
        tempfile.mkdtemp(
            prefix="caf-captions-smoke-", dir=str(root / "sandbox" / "output")
        )
    )
    ...
    proc = subprocess.run(
        [
            sys.executable,
            "-m",
            "repo.tools.validate_captions_artifact",
            str(root / "repo" / "examples" / "captions_artifact.v1.example.json"),
        ],
    ...
    if not srt_out.exists() or srt_out.stat().st_size == 0:
        print(
            "ERROR: expected non-empty final.srt from captions artifact",
            file=sys.stderr,
        )
        return 1
    ...
    if status_b != "skipped_external_missing":
        print(
            f"ERROR: expected skipped_external_missing, got {status_b}", file=sys.stderr
        )
        return 1
