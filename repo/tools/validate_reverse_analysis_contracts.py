...
def _validate_semantics(
    ...
    for i, shot in enumerate(shots):
        ...
        if isinstance(camera, str) and camera == "unknown":
            if "camera_confidence" in shot and float(shot.get("camera_confidence", 0.0)) > 0.0:
                errs.append(f"truth.shots[{i}].camera=unknown must not have positive camera_confidence")
        ...
        if ph is not None and not (isinstance(ph, str) and ph.startswith("#") and len(ph) == 7):
    ...
    if isinstance(visual_facts, dict):
        ...
        if mode == "unknown" and isinstance(conf, (int, float)) and float(conf) > 0:
            errs.append("truth.visual_facts.camera_movement_mode=unknown requires camera_movement_confidence=0")
        ...
        if isinstance(palette, list):
            for i, item in enumerate(palette):
                if not (isinstance(item, str) and item.startswith("#") and len(item) == 7):
                    errs.append(f"truth.visual_facts.palette_top_hex[{i}] must be #RRGGBB")
        if isinstance(mode, str) and mode != "unknown":
            for i, shot in enumerate(shots):
                ...
                if isinstance(shot_mode, str) and shot_mode not in (mode, "unknown"):
                    errs.append(f"truth.shots[{i}].camera must align with truth.visual_facts.camera_movement_mode")
    ...
    keyframe_times = [float(x.get("t_sec", 0.0)) for x in keyframe_doc.get("keyframes", [])]
    ...
    for i, source in enumerate(vendor_sources):
        ...
        if not relpath.startswith("repo/analysis/vendor/"):
            errs.append(f"suggestions.vendor_sources[{i}].relpath must stay under repo/analysis/vendor/")
    ...
    if measured_truth < inferred:
        errs.append("confidence.measured_truth should be >= confidence.inferred_semantics for truth-first posture")
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument("--pose", required=True, help="Path to pose_checkpoints.v1 JSON")
    ...
    if not errors:
        errors.extend(_validate_semantics(docs["reverse"], docs["beat"], docs["pose"], docs["keyframes"]))
