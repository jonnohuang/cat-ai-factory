...
def validate_semantics(manifest_data: Dict[str, Any]) -> bool:
    ...
    # Identity check
    if manifest_data.get("project") != "Cat AI Factory" or manifest_data.get("schema") != "audio_manifest.v1":
         eprint("SEMANTIC_ERROR: Manifest appears to be invalid (wrong project or schema).")
         ok = False
    ...
    for i, bed in enumerate(beds):
        ...
        if relpath:
            ...
            if os.path.isabs(norm_rel) or norm_rel.startswith("/"):
                eprint(f"SEMANTIC_ERROR: beds[{i}].relpath '{relpath}' must not be absolute")
                ok = False
            
            # Must not contain traversal (check segments)
            path_segments = norm_rel.split("/")
            if ".." in path_segments:
                eprint(f"SEMANTIC_ERROR: beds[{i}].relpath '{relpath}' must not contain path traversal (..)")
                ok = False
                
            # Must start with assets/audio/
            if not norm_rel.startswith("assets/audio/"):
                 eprint(f"SEMANTIC_ERROR: beds[{i}].relpath '{relpath}' must start with 'assets/audio/'")
                 ok = False
...
def main(argv: List[str]) -> int:
    if len(argv) != 2:
        eprint("Usage: python3 repo/tools/validate_audio_manifest.py sandbox/assets/audio/audio_manifest.v1.json")
        return 1

    manifest_path = argv[1]
    
    # 1. Load schema
    schema_path = os.path.join(_REPO_ROOT, "sandbox", "assets", "audio", "audio_manifest.v1.schema.json")
