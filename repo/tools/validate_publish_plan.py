...
def check_lineage(data, filepath):
    ...
    if job_id != parent_dir:
        print(
            f"LINEAGE ERROR: job_id '{job_id}' does not match parent directory '{parent_dir}'"
        )
        return False
    return True
    ...
    for platform, plan in platforms.items():
        ...
        for i, clip in enumerate(plan.get("clips", [])):
            ...
            if cid:
                if cid in clip_ids:
                    print(
                        f"SEMANTIC ERROR: Duplicate clip id '{cid}' in platform '{platform}'"
                    )
                    has_error = True
                clip_ids.add(cid)
...
def main():
    ...
    schema_path = os.path.join(
        os.path.dirname(__file__), "../shared/publish_plan.schema.json"
    )
    try:
