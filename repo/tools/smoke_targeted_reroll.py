...
def run_orchestrator(job_id: str, inject_failure: bool = False):
    ...
    cmd = [
        "/opt/miniconda3/envs/cat-ai-factory/bin/python", "-m", "repo.services.orchestrator.ralph_loop",
        "--job", str(root / "sandbox" / "jobs" / f"{job_id}.job.json"),
        "--max-retries", "2"
    ]
...
def main():
    ...
    worker_log_path = root / "sandbox" / "logs" / job_id / "attempts" / "run-0002" / "worker.log"
    if worker_log_path.exists():
        ...
        if "shot_id='shot_2'" in log_content or "CAF_TARGET_SHOT_ID" in log_content:
            print("SUCCESS: Targeted re-roll detected in worker log!")
             ...
             print("ERROR: Worker log for run-0002 did not indicate shot_2 was targeted.")
             sys.exit(1)
    else:
