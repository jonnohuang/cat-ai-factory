...
def test_pointer_authority():
    ...
    brief = {"motion": "dance loop", "prompt": "Mochi dino dancing in the living room"}

    # 1. Test Standard Resolution
    res = resolver.resolve(
        "authority-test-v1", brief, policy="prefer_canon_strict_motion"
    )
    ...
    assert (
        "strict" in pointers["quality_target"]
    ), "Should have picked strict target per policy"
