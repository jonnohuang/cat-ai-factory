...
def main(argv: list[str]) -> int:
    ...
    derive_cmd = [sys.executable, "-m", "repo.tools.derive_two_pass_orchestration", "--job-id", job_id]
    print("RUN:", " ".join(derive_cmd))
    subprocess.check_call(derive_cmd, cwd=str(root))

    two_pass_path = root / "sandbox" / "logs" / job_id / "qc" / "two_pass_orchestration.v1.json"
    val_two_pass_cmd = [sys.executable, "-m", "repo.tools.validate_two_pass_orchestration", str(two_pass_path)]
    ...
    quality_decision_path = root / "sandbox" / "logs" / job_id / "qc" / "quality_decision.v1.json"
    val_quality_cmd = [sys.executable, "-m", "repo.tools.validate_quality_decision", str(quality_decision_path)]
    ...
    if not isinstance(two_pass_rel, str) or not two_pass_rel.startswith(f"sandbox/logs/{job_id}/qc/"):
        print("ERROR: decision missing two_pass_orchestration_relpath", file=sys.stderr)
        return 1

    action = str(decision.get("decision", {}).get("action", ""))
    if action not in {"proceed_finalize", "retry_motion", "retry_recast", "block_for_costume", "escalate_hitl"}:
        print(f"ERROR: unexpected action {action!r}", file=sys.stderr)
        return 1

    print("OK:", two_pass_path)
    print("OK:", quality_decision_path)
    print("next_preferred_action:", two_pass.get("orchestration", {}).get("next_preferred_action"))
