...
def setup_mock_job(job_id, quality_targets_relpath=None):
    ...
    job_data = {
        ...
        "job_id": job_id,
        "quality_target": {
            "relpath": quality_targets_relpath
        },
        "pointer_resolution": {
...
def run_decide(job_id, max_retries=2):
    cmd = [sys.executable, str(DECIDE_TOOL), "--job-id", job_id, "--max-retries", str(max_retries)]
    env = os.environ.copy()
    env["CAF_QC_DEBUG"] = "1"
    ...
    if not decision_path.exists():
         return None, f"Decision file missing. STDOUT: {result.stdout}, STDERR: {result.stderr}"
...
def test_strict_routing():
    ...
    gates = [
        {"metric": m, "dimension": "motion" if m in ["temporal_stability", "loop_seam", "background_stability"] else "identity", "status": "pass"}
        for m in ["identity_consistency", "mask_edge_bleed", "temporal_stability", "loop_seam", "audio_video", "background_stability", "identity_drift"]
    ]
    # Set one to unknown
    ...
    assert decision["action"] == "retry_motion", f"Expected retry_motion due to unknown gate, got {decision['action']}"
    ...
    setup_mock_job(job_id)
    setup_mock_report(job_id, gates=[
        {"metric": "temporal_stability", "dimension": "motion", "status": "pass"}
        # missing identity_consistency which is in DEFAULT_QUALITY_TARGETS
    ])
    payload, err = run_decide(job_id)
    decision = payload["decision"]
    assert decision["action"] == "escalate_hitl", f"Expected escalate_hitl due to missing metric, got {decision['action']}"
    assert "missing mandatory metrics" in decision["reason"], f"Expected reason to mention missing metrics, got {decision['reason']}"
    ...
    if decision["action"] != "escalate_hitl":
        print(f"DEBUG: reason={decision['reason']}")
        print(f"DEBUG: stdout/err={err}")
    assert decision["action"] == "escalate_hitl", f"Expected escalate_hitl due to budget limit, got {decision['action']}"
    assert "budget exceeded" in decision["reason"], f"Expected reason to mention budget, got {decision['reason']}"
...
if __name__ == "__main__":
    try:
        ...
        for jid in ["smoke-qc-unknown", "smoke-qc-missing-metric", "smoke-qc-budget"]:
            ...
            if logs_dir.exists(): shutil.rmtree(logs_dir)
            # Cleanup tmp qt files
            qt_path = repo_root / "repo" / "shared" / f"tmp_qt_{jid}.v1.json"
            if qt_path.exists(): qt_path.unlink()
