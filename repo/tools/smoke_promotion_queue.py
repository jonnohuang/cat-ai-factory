...
def main() -> int:
    ...
    candidate_good = {
        ...
        "evidence": {"quality_lift": {"pass_rate_delta": 0.1, "retry_count_delta": -1}},
    }
    ...
    candidate_bad = {
        **candidate_good,
        ...
        "evidence": {
            "quality_lift": {"pass_rate_delta": -0.01, "retry_count_delta": 1}
        },
    }
    ...
    approved_good = [
        row
        for row in approved
        if isinstance(row, dict) and row.get("candidate_id") == "smoke-candidate-good"
    ]
    approved_bad = [
        row
        for row in approved
        if isinstance(row, dict) and row.get("candidate_id") == "smoke-candidate-bad"
    ]

    if len(approved_good) != 1:
        print(
            f"ERROR: expected exactly one good promotion approval, found {len(approved_good)}",
            file=sys.stderr,
        )
        return 1
    if approved_bad:
    ...
    if queue_rem:
        print(
            f"ERROR: queue should be empty after processing, found {len(queue_rem)}",
            file=sys.stderr,
        )
        return 1
