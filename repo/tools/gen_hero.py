...
def _init_client() -> genai.Client:
    ...
    print(f"DEBUG: Initializing client project={project} location={location} creds={bool(creds)}")
    return genai.Client(vertexai=True, project=project, location=location)

def _generate_image(client: genai.Client, prompt: str, output_path: pathlib.Path) -> None:
...
def main():
    ...
    parser.add_argument("--costume", required=True, help="Costume ID from registry (or match hero default)")
    ...
    parser.add_argument("--costume-path", default="repo/shared/costume_profiles.v1.json")
    parser.add_argument("--style", help="Style prompt override (e.g. 'Pixar-style 3D, fluffy fur')")
    parser.add_argument("--offset-y", type=int, default=0, help="Vertical offset hint (no-op for Imagen)")
    ...
    # Resolve Hero
    hero = next((h for h in hero_data.get("heroes", []) if h["hero_id"] == args.hero), None)
    if not hero:
        print(f"Error: Hero {args.hero} not found")
        return 1
        
    # Resolve Costume
    costume_entry = next((c for c in costume_data.get("profiles", []) if c["id"] == args.costume), None)
