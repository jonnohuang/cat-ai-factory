...
def main(argv: list[str]) -> int:
    _ = argv
    env = dict(os.environ)
    env["CAF_ENGINE_ROUTE_MODE"] = "lab"
    cmd = [
        sys.executable,
        "-m",
        "repo.services.planner.planner_cli",
        "--prompt",
        "Mochi meta ai lab provider smoke",
        "--provider",
        "meta_ai_lab",
        "--inbox",
        "sandbox/inbox",
    ...
    if rc != 0:
        print("ERROR: planner_cli failed for meta_ai_lab smoke", file=sys.stderr)
        return 1
    ...
    if vrc != 0:
        print("ERROR: generated meta_ai_lab job failed validate_job", file=sys.stderr)
        return 1
    print("OK: planner meta_ai_lab provider smoke")
