...
def main():
    ...
    resolution_dance = resolver.resolve(
        job_id, dance_brief, policy="prefer_canon_strict_motion"
    )

    rejected = resolution_dance.get("rejected_candidates", [])
    print(f"  Rejected Count: {len(rejected)}")
    dance_missing = any(
        r["reason"].startswith("dance_context_missing") for r in rejected
    )
    ...
    if dance_missing:
        ...
        print(
            f"  WARNING: Did not find expected dance rejections. Pointers: {resolution_dance['pointers'].keys()}"
        )
