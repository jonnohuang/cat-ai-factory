...
def main():
    parser = argparse.ArgumentParser(
        description="Verify pose divergence between reference and generated video."
    )
    ...
    parser.add_argument(
        "--threshold", type=float, default=0.7, help="Similarity threshold for PASS"
    )
    ...
    try:
        ...
        result = {
            ...
            "frames_compared": min(
                len(ref.get("sequence", [])), len(gen.get("sequence", []))
            ),
        }
