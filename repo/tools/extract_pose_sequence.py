...
def extract_pose_sequence(video_path: pathlib.Path, output_json: pathlib.Path, fps: float = 24.0):
    print(f"Extracting pose from {video_path}...")
    
    if not HAS_CV2:
        print("WARNING: cv2/mediapipe not found. Emitting mock pose sequence.", file=sys.stderr)
        pose_sequence = [{"frame_idx": i, "timestamp_sec": round(i/fps, 4), "landmarks": []} for i in range(24)]
        output_data = {
            "version": "pose_seq.v1",
    ...
    print(f"Successfully saved {len(pose_sequence)} frames of pose data to {output_json}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Extract MediaPipe pose landmarks from video.")
