...
def compute_identity_drift(video_path: pathlib.Path, ref_image: pathlib.Path) -> float:
    ...
    ref_hist = cv2.calcHist(
        [ref_img], [0, 1, 2], None, [8, 8, 8], [0, 256, 0, 256, 0, 256]
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Compute granular CV metrics for video quality."
    )
    parser.add_argument("video_path", type=pathlib.Path)
    parser.add_argument(
        "--ref-image", type=pathlib.Path, help="Reference image for identity drift"
    )
    parser.add_argument(
        "--output", type=pathlib.Path, help="Append to this JSON report"
    )
    ...
    results = {"background_stability": stability, "identity_drift": drift}
