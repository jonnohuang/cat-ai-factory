...
class InstagramAdapter(PublisherAdapter):
    ...
    def generate_checklist_content(self, publish_plan: Dict[str, Any]) -> str:
        ...
        clips = (
            publish_plan.get("platform_plans", {}).get("instagram", {}).get("clips", [])
        )
        for i, clip in enumerate(clips):
            ...
            lines.append(
                f"[ ] 3.{clip_idx}.1 Select 'clips/{clip_name}/video/final.mp4'."
            )
            lines.append(f"[ ] 3.{clip_idx}.2 Add Cover (if planned).")
            lines.append(
                f"[ ] 3.{clip_idx}.3 Copy Caption from 'clips/{clip_name}/copy/copy.en.txt'."
            )
            lines.append(
                f"[ ] 3.{clip_idx}.4 Add Music/Audio: See 'clips/{clip_name}/audio/audio_notes.txt'."
            )
            lines.append(
                f"[ ] 3.{clip_idx}.5 Tag accounts/location/products if planned."
            )
