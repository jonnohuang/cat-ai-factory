...
class YouTubeAdapter(PublisherAdapter):
    ...
    def generate_checklist_content(self, publish_plan: Dict[str, Any]) -> str:
        ...
        for i, clip in enumerate(clips):
            ...
            lines.append(
                f"[ ] 3.{clip_idx}.1 Drag & Drop 'clips/{clip_name}/video/final.mp4'."
            )
            lines.append(
                f"[ ] 3.{clip_idx}.2 Copy Title from 'clips/{clip_name}/copy/copy.en.txt' (or correct language)."
            )
            lines.append(f"[ ] 3.{clip_idx}.3 Copy Description from same file.")
            lines.append(
                f"[ ] 3.{clip_idx}.4 Set Visibility to 'Public' (or as planned)."
            )
            lines.append(f"[ ] 3.{clip_idx}.5 Select 'Not made for kids'.")
            lines.append(f"[ ] 3.{clip_idx}.6 Add Tags from plan if defined.")
            lines.append(
                f"[ ] 3.{clip_idx}.7 Check 'clips/{clip_name}/audio/audio_notes.txt' for audio details."
            )
