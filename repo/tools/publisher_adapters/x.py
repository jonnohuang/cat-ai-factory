...
class XAdapter(PublisherAdapter):
    ...
    def generate_checklist_content(self, publish_plan: Dict[str, Any]) -> str:
        ...
        for i, clip in enumerate(clips):
            ...
            lines.append(f"[ ] 3.{clip_idx}.1 Attach Media 'clips/{clip_name}/video/final.mp4'.")
            lines.append(f"[ ] 3.{clip_idx}.2 Copy Text from 'clips/{clip_name}/copy/copy.en.txt'.")
            lines.append(f"[ ] 3.{clip_idx}.3 Add Alt Text (recommended).")
            lines.append(f"[ ] 3.{clip_idx}.4 Review 'clips/{clip_name}/audio/audio_notes.txt' for any required audio guidance.")
