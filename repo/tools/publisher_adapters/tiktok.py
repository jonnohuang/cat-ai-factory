...
class TikTokAdapter(PublisherAdapter):
    ...
    def generate_checklist_content(self, publish_plan: Dict[str, Any]) -> str:
        ...
        clips = (
            publish_plan.get("platform_plans", {}).get("tiktok", {}).get("clips", [])
        )
        for i, clip in enumerate(clips):
            ...
            lines.append(
                f"[ ] 3.{clip_idx}.2 Add Sound: See 'clips/{clip_name}/audio/audio_notes.txt'."
            )
            lines.append(
                f"[ ] 3.{clip_idx}.3 Copy Description from 'clips/{clip_name}/copy/copy.en.txt'."
            )
