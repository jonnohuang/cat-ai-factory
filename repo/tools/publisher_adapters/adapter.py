    ...
    if isinstance(data, dict):
        for key, value in data.items():
            ...
            if SECRET_REGEX.search(key):
                raise ValueError(
                    f"SECURITY ERROR: Potential secret found in key: {current_path}"
                )
...
class SharedBundleBuilder:
    ...
    def _validate_job_id(job_id: str) -> None:
        ...
        if ".." in job_id or "/" in job_id or "\\" in job_id:
            raise ValueError(
                f"Security Error: Invalid job_id '{job_id}'. Must not contain path separators or traversal."
            )

        if not re.match(r"^[a-zA-Z0-9_\-\.]+$", job_id):
            raise ValueError(
                f"Security Error: Invalid characters in job_id '{job_id}'."
            )
    ...
    def build_bundle(
        ...
        if resolved_dist != expected_dist_root:
            raise ValueError(
                f"Security Error: dist_root must resolve to '{expected_dist_root}', got: {resolved_dist}"
            )
        ...
        if not SharedBundleBuilder._is_relative_to(bundle_root_final, resolved_dist):
            raise ValueError(
                f"Security Error: computed path {bundle_root_final} escapes dist_root"
            )
        ...
        try:
            ...
            for idx, clip_meta in enumerate(clips_meta):
                ...
                expected_output_root = (
                    repo_root / "sandbox" / "output" / job_id
                ).resolve()

                # Handle /sandbox/... or sandbox/... inputs mapping to repo_root
                if src_video_str.startswith("/sandbox/") or src_video_str.startswith(
                    "sandbox/"
                ):
                ...
                # Validate strictness
                if not SharedBundleBuilder._is_relative_to(
                    abs_src_video, expected_output_root
                ):
                    raise ValueError(
                        f"Security Error: source video '{src_video_str}' must be a valid sandbox path (e.g. 'sandbox/output/{job_id}/final.mp4'). Absolute host paths are discouraged."
                    )

                if not abs_src_video.exists():
                    raise FileNotFoundError(
                        f"Required video artifact not found: {abs_src_video}"
                    )
                ...
                for lang in ["en", "zh-Hans"]:
                    ...
                    with open(
                        clip_dir / "copy" / f"copy.{lang}.txt", "w", encoding="utf-8"
                    ) as f:
                        f.write(content)
                ...
                with open(
                    clip_dir / "audio" / "audio_plan.json", "w", encoding="utf-8"
                ) as f:
                ...
                with open(
                    clip_dir / "audio" / "audio_notes.txt", "w", encoding="utf-8"
                ) as f:
                    f.write(audio_notes)
                ...
                for asset_path in audio_assets:
                    ...
                    if asset_path.startswith("/sandbox/") or asset_path.startswith(
                        "sandbox/"
                    ):
                    ...
                    if not SharedBundleBuilder._is_relative_to(abs_asset, sandbox_root):
                        raise ValueError(
                            f"Security Error: asset {asset_path} must be in sandbox/"
                        )

                    if abs_asset.exists():
                        asset_name = abs_asset.name
                        shutil.copy2(
                            abs_asset, clip_dir / "audio" / "assets" / asset_name
                        )
                    else:
                        raise FileNotFoundError(
                            f"Required audio asset not found: {asset_path}"
                        )
            ...
            try:
                ...
                if (
                    bundle_root_old
                    and bundle_root_old.exists()
                    and not bundle_root_final.exists()
                ):
                    try:
...
class PublisherAdapter(abc.ABC):
    ...
    def generate_bundle(
        ...
        dist_root: str = "sandbox/dist_artifacts",
    ) -> Optional[Path]:
        """
        Main entry point.
        """
        ...
        if plan.get("job_id") != job_id:
            raise ValueError(
                f"Plan job_id '{plan.get('job_id')}' does not match requested '{job_id}'"
            )
