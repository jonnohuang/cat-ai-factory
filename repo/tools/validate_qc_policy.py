...
def main(argv: list[str]) -> int:
    if len(argv) != 2:
        eprint("Usage: python -m repo.tools.validate_qc_policy path/to/qc_policy.v1.json")
        return 1
    ...
    root = _repo_root()
    schema = _load(root / "repo" / "shared" / "qc_policy.v1.schema.json")
    data = _load(target)
    try:
    ...
    if len(gate_ids) != len(set(gate_ids)):
        eprint("SEMANTIC_ERROR: gate_id values must be unique")
        return 1
