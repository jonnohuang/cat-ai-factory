...
def validate_references(ledger_data: Dict[str, Any], registry_path: str, bible_path: Optional[str]) -> bool:
    ok = True
    ...
    if not isinstance(heroes, list):
        eprint(f"ERROR: Invalid registry format in {registry_path}: 'heroes' must be a list")
        return False
    ...
    if bible_path and os.path.exists(bible_path):
        ...
        # Identity check
        if bible_data.get("project") != "Cat AI Factory" or bible_data.get("schema") != "series_bible.v1":
             eprint(f"SEMANTIC_ERROR: Series Bible at {bible_path} appears to be invalid (wrong project or schema).")
             ok = False
        
        # Pre-compute valid settings w/ shape check
        settings = bible_data.get("settings")
        if not isinstance(settings, list):
            eprint(f"ERROR: Invalid bible format in {bible_path}: 'settings' must be a list")
            return False
            
        valid_settings = {s.get("setting_id") for s in settings if isinstance(s, dict) and s.get("setting_id")}

        # Pre-compute valid gags w/ shape check
        gags = bible_data.get("running_gags")
        if not isinstance(gags, list):
            eprint(f"ERROR: Invalid bible format in {bible_path}: 'running_gags' must be a list")
            return False
            
        valid_gags = {g.get("gag_id") for g in gags if isinstance(g, dict) and g.get("gag_id")}
        
    elif bible_path:
        eprint(f"Warning: Series Bible not found at {bible_path}. Skipping continuity checks.")

    # 3. Check Episodes
    episode_ids = set()
    episodes = ledger_data.get("episodes", [])
    if not isinstance(episodes, list):
        eprint(f"ERROR: Invalid ledger format: 'episodes' must be a list")
        return False
    
    for i, episode in enumerate(episodes):
        if not isinstance(episode, dict):
             eprint(f"SEMANTIC_ERROR: Invalid episode format at index {i}: must be an object")
             ok = False
             continue
        ...
        if isinstance(heroes_involved, list):
            for j, hero_ref in enumerate(heroes_involved):
                if not isinstance(hero_ref, str):
                    eprint(f"SEMANTIC_ERROR: episodes[{i}].heroes_involved[{j}] must be a string")
                    ok = False
                elif hero_ref not in hero_ids:
                    eprint(f"SEMANTIC_ERROR: episodes[{i}].heroes_involved[{j}] '{hero_ref}' not found in registry {registry_path}")
                    ok = False
            ...
            if isinstance(links, dict):
                ...
                if "setting_id" in links:
                    ...
                    if setting_ref not in valid_settings:
                         eprint(f"SEMANTIC_ERROR: episodes[{i}].continuity_links.setting_id '{setting_ref}' not found in bible {bible_path}")
                         ok = False
                    ...
                    if isinstance(gags, list):
                        for k, gag_ref in enumerate(gags):
                            if not isinstance(gag_ref, str):
                                eprint(f"SEMANTIC_ERROR: episodes[{i}].continuity_links.gags_used[{k}] must be a string")
                                ok = False
                            elif gag_ref not in valid_gags:
                                 eprint(f"SEMANTIC_ERROR: episodes[{i}].continuity_links.gags_used[{k}] '{gag_ref}' not found in bible {bible_path}")
                                 ok = False
...
def main(argv: List[str]) -> int:
    if len(argv) < 2 or len(argv) > 4:
        eprint("Usage: python3 repo/tools/validate_episode_ledger.py path/to/ledger.v1.json [path/to/hero_registry.v1.json] [path/to/series_bible.v1.json]")
        return 1
    ...
    # 1. Load schema
    schema_path = os.path.join(_REPO_ROOT, "repo", "shared", "episode_ledger.v1.schema.json")
