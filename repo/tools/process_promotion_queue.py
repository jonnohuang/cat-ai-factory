...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Process promotion_queue.v1 artifact into promotion registry.")
    parser.add_argument("--queue-relpath", default="repo/shared/promotion_queue.v1.json")
    ...
    if not isinstance(queue_data, dict) or queue_data.get("version") != "promotion_queue.v1":
        # If queue missing or invalid, treat as empty but don't crash
        print(f"WARNING: Promotion queue not found or invalid: {args.queue_relpath}", file=sys.stderr)
        queue_data = {"version": "promotion_queue.v1", "generated_at": _utc_now(), "queue": []}
    ...
    queue_schema = _load(root / "repo" / "shared" / "promotion_queue.v1.schema.json") or {}
    candidate_schema = _load(root / "repo" / "shared" / "promotion_candidate.v1.schema.json") or {}
    ...
    print(f"Processed {processed} candidates. Approved {approved_count}. Wrote {out_path}")
    return 0
